...
from booking_app_fastapi.database.models import Property, PropertyImage
from booking_app_fastapi.database.schema import (PropertyInSchema,PropertyImageInSchema,
                                                 PropertyOutSchema,PropertyImageOutSchema)
...
async def create_property(property_data:PropertyInSchema, db:Session= Depends(get_db)):
...
async def update_property(property_id:int, property_data:PropertyInSchema, db:Session= Depends(get_db)):
    property_db = db.query(Property).filter(Property.id == property_id).first()
    if property_db is None:
        raise HTTPException(status_code=400, detail="Property not found")
...
@property_image_router.get("/{image_id}",response_model=PropertyOutSchema)
...
async def update_image(image_id:int, image_data:PropertyImageInSchema, db:Session= Depends(get_db)):
